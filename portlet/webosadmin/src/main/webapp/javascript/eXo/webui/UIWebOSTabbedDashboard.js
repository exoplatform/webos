...
eXo.webui.UIWebOSTabbedDashboard = {
	...
	renameTabLabel : function(e){
		...
		//If user presses on the ESCAPE key, then reset the span element on the tab
			...
			var inputElement = eXo.core.Browser.getEventSource(e);
			if(eXo.webui.UIWebOSTabbedDashboard.backupElement) {
 				inputElement.parentNode.replaceChild(eXo.webui.UIWebOSTabbedDashboard.backupElement, inputElement);
 				eXo.webui.UIWebOSTabbedDashboard.backupElement = null;
			}
		}
	},
	...
	showEditLabelInput : function(selectedElement, nodeName, currentContent){
		...
		inputElement.onkeypress = eXo.webui.UIWebOSTabbedDashboard.renameTabLabel;
		inputElement.setAttribute('maxLength', 50);
		inputElement.onblur = function() {
			prNode.replaceChild(eXo.webui.UIWebOSTabbedDashboard.backupElement, inputElement);
		};
	...
	createDashboardPage : function(e){
		...
		if(eXo.webui.UIWebOSTabbedDashboard.isInRequest) return;
		var keyNum = e.keyCode;
		...
		if(keyNum == 13){
			...
			eXo.webui.UIWebOSTabbedDashboard.isInRequest = true;
			window.location = href;
		}
	...
	createTabDashboard : function(addTabElement){
		...
		inputElement.onkeypress = eXo.webui.UIWebOSTabbedDashboard.createDashboardPage;
		inputElement.onblur = eXo.webui.UIWebOSTabbedDashboard.cancelTabDashboard;
		inputElement.setAttribute('maxLength', 50);
